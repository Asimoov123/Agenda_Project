    ...
    while (current0Cell != NULL) {
        if (current0Cell != currentLvlCell){ // If the cell on the first line is different from the one on the line we want to display
            while (current0Cell != currentLvlCell){ // while it is true, display '-' instead of cell
                printf("---%.*s------", numPlaces(current0Cell->value), "----------"); // https://stackoverflow.com/questions/14678948/how-to-repeat-a-char-using-printf
    ...
    if (mylist->heads[0]->value > val) { // addHead if the value we want to insert is smaller than the first element
        ...
        for (int i = newCell->level-1; i >= 0; i--){ // going from the highest lvl of the new cell to the first level
            ...
            if (current == NULL){
                ...
                while (current->next[i] != NULL && current->value < newCell->value) { // while current cell's value is smaller than the new cell's value
                ...
                if (current->next[i] == NULL && current->value < newCell->value) // it the successor is null (meaning the value is higher than every value in the list)
                    update[i] = current; // save the current cell
...
int isValueInListMultiniveaux(t_d_list mylist, int val){
    ...
    if (mylist.heads[0]->value == val) { // addHead if the value we want to insert is smaller than the first element
        ...
        while (current_lvl > 0 && current != NULL){
            while (current->next[current_lvl] != NULL && current->value < val) { // while current cell's value is smaller than the new cell's value
...
int isValueInListLvl1(t_d_list mylist, int val){
    ...
    if (mylist.heads[0]->value == val) { // addHead if the value we want to insert is smaller than the first element
        ...
        for (int i = mylist.max_level-1; i >= 0; i--){ // going from the highest lvl of the new cell to the first level
            current = mylist.heads[i]; // set current to head
            if (current != NULL){
                while (current->next[i] != NULL && current->value < val) { // while current cell's value is smaller than the new cell's value
                    current = current->next[i]; // set current cell to its successor (on the same line)
                }
                if (current->value == val){
                    return 1;
